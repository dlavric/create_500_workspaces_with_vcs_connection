...
for WORKSPACE in $WORKSPACES; do
  ...
  --header "Authorization: Bearer $TOKEN" \
  --header "Content-Type: application/vnd.api+json" \
  https://$TFE_HOST/api/v2/workspaces/$WORKSPACE/runs?filter%5Boperation%5D=plan_and_apply | jq -r '.data[].id'`


#   "https://$TFE_HOST/api/v2/organizations/$ORG_NAME/workspaces?page%5Bsize%5D=$PAGE_SIZE&page%5Bnumber%5D=$page"
  ...
  for RUN in $RUNS; do
    ...
    https://$TFE_HOST/api/v2/runs/$RUN | jq -r '.data.attributes."created-at"'`
     ...
     --header "Authorization: Bearer $TOKEN" \
     --header "Content-Type: application/vnd.api+json" \
     https://$TFE_HOST/api/v2/runs/$RUN | jq -r '.data.attributes.message'`
    
     echo "    RUN DATE: $RUN_DATE" - RUN NAME: $RUN_NAME >> $OUTPUT_FILE
